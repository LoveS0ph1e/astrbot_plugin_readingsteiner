...
# ── 连接默认值 ──
# 默认指向同 docker 网络的服务名；非容器/单机改 127.0.0.1:8000。
...
# ── EverOS profile_data 结构字段（summary/explicit_info/implicit_traits）──
# injection 渲染与 profile_quality 校验共用，避免硬编码字符串散落（单一事实源）。
