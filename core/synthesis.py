...
请**只依据**这些信息，用你的视角，为这个人写一段整体印象——不超过 {max_chars} 字，可以是二三句连贯的话，
不必强行压缩成一句；但仍要是「整体印象」而非罗列：抓住这个人最核心的样子（性格底色 / 与你的关系基调），
而非堆砌事实、不分点、不复述清单。只能基于给定信息落笔，绝不新增未提及的事实或主观臆测。
...
def _clean(raw: str, max_chars: int) -> str:
    """清洗 LLM 输出：去首尾空白/引号、去「整体印象：」类前缀、合并多行、宽容截断。

    现在允许一段（二三句）而非强制一句，故不再只取首行——多行合并成一段，
    段内换行折成空格，避免注入时被误判为多个条目。
    """
