...
### Changed
- **整体印象合成优化**：`core/synthesis.py` 的 `overall_impression` 从强制"一句话"
  改为允许"一段话（二三句）"——之前一句话过于简略抽象，放宽为一段后能更自然地
  抓住性格底色与关系基调，同时保持紧凑（≤80字上限不变）。同步去掉 `_clean()` 的
  "只取首行"截断，改为多行合并成段。6 条单测更新通过，插件全量 130 测试无回归。

## [v0.5.1] - 2026-06-30

### Fixed
- **群聊张冠李戴（记忆归属）**：注入的 `ReadingSteiner_Memory` 块此前不带用户归属标签，人格身份
  锚定规则（§11：要求记忆条目带 `user: <id>` 标签、仅采纳 `user==当前对话者` 者）因而无从校验、
  只能「猜归属」——群聊多人快速交替时易把他人/助手的记忆错绑到当前对话者，且随人格 §11 趋严而
  更易高发。现注入块首行加 `user: <QQ号>` 归属声明（本插件只检索/注入当前用户本人记忆，故整块
  均归属此人），人格据此可 `user==<<CURRENT_USER>>` 确信采纳。受新增 `inject_user_attribution`
  （默认开）控制。
